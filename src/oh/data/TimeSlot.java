...
public class TimeSlot {
    ...
    SimpleDateFormat time_convertor = new SimpleDateFormat("hh:mma", Locale.US);
    public void filter(String str_start, String str_send){
        try {
            Date start = null, end = null;
            if(str_start != null && str_start.length() > 0){
                start = time_convertor.parse(str_start);
            }
            if(str_send != null && str_send.length() > 0){
                end = time_convertor.parse(str_send);
            }
            Date cur_start = time_convertor.parse(this.startTime.get());
            Date cur_end = time_convertor.parse(this.endTime.get());
            if(start != null && end != null){
                if(cur_start.compareTo(start) >= 0 && cur_end.compareTo(end) <= 0){
                    for (DayOfWeek dow : DayOfWeek.values()) {
                        dayText.get(dow).set(getMyDayText(dow));
                    }
                }else{
                    for (DayOfWeek dow : DayOfWeek.values()) {
                        dayText.get(dow).set("");
                ...
                if(cur_start.compareTo(start) >= 0){
                    for (DayOfWeek dow : DayOfWeek.values()) {
                        dayText.get(dow).set(getMyDayText(dow));
                    }
                }else{
    ...
    public String getMyDayText(DayOfWeek dow){
